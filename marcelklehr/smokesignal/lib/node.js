...
function Node(options) {
    ...
    for (var prop in this.options)
        if ('undefined' == typeof this.options[prop])
            throw new Error('"' + prop + '" option is not set');
    ['trace', 'debug', 'info', 'warn', 'error', 'fatal'].forEach(function (method) {
        if ('function' == typeof this.options.logger[method])
            return;
    ...
    this.interval = setInterval(function () {
        ...
        if (this.options.seeds.length) {
            this.options.seeds.forEach(function (seed) {
                if (seed.port == this.options.port && seed.address == this.options.address)
                    return;
...
Node.prototype.enterNetwork = function () {
    if (!this.options.seeds)
        return this.logger.warn('Would have tried to enter network, but no seeds specified');
    if (this.options.seeds.length) {
        this.options.seeds.forEach(function (seed) {
            if (seed.port == this.options.port && seed.address == this.options.address)
                return;
...
Node.prototype.onUnconnectedPeerRequest = function (socket, data) {
    ...
    if (!data.id)
        return;
    if (!net.isIP(data.remoteAddress))
        return;
    if (typeof (data.remotePort) != 'number')
        return;
...
Node.prototype.onPeerRequest = function (data) {
    ...
    if (this.peers.isFull())
        return;
    if (this.peers.inList(data.id))
        return;
    if (this.peers.isSufficientlyFilled() && Math.random() > 0.5)
        return; // Reject a half of all pull requests
...
Node.prototype.onPing = function (packet, peer) {
    ...
    if (!net.isIP(data.targetAddress))
        return;
    if (!net.isIP(data.remoteAddress))
        return;
...
Node.prototype.onPong = function (socket, data) {
    ...
    if (typeof (data.remotePort) != 'number')
        return;
    if (!this.outstandingPongs[data.respondsTo])
        return;
